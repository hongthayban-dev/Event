...
const MAXSEG_        = 16; // จำนวน segment สูงสุดบนวงล้อ
...
function getPublicSettings_() {
  ...
  var hide = ['pin_lottery', 'pin_remote', 'discount_code']; // ห้ามส่ง pin และ discount code ออกหน้าเว็บ
  var out = {};
...
function createOrder_(p) {
  ...
  if (p.line_user_id) {
    ...
    if (discountPercent > 0) {
      var discountAmt = Math.round(subtotal * discountPercent / 100);
      discountLine = '\n🏷️ ส่วนลด ' + discountPercent + '%: -฿' + discountAmt.toLocaleString('th-TH');
    }
...
function prepare_(p) {
  ...
  var display = shuffle_(pool.slice()).slice(0, MAXSEG_).map(function (r) {
...
function requestSpin_(p) {
  ...
  // ชุดชื่อสำหรับแสดงบนวงล้อ: ผู้ชนะ + สุ่มคนอื่นมาเติม รวมไม่เกิน MAXSEG_ ช่อง
  var display = winners.map(function (w) { return { id: w.reg_id, name: w.nickname || w.first_name || '—' }; });
  var fillers = shuffle_(work.slice());
  for (var j = 0; j < fillers.length && display.length < MAXSEG_; j++) {
